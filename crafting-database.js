(function() {
  ...
  const CraftingDatabase = () => {
    ...
    return h('div', { className: 'container mx-auto px-4 max-w-7xl' },
      // Top action buttons with minimal top padding and more bottom padding
      h('div', { className: 'flex flex-wrap gap-2 items-center justify-end pt-4 pb-10' },
        h('button', {
          onClick: () => setIsManagingClassStats(true),
          className: 'px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-sm',
          style: { backgroundColor: '#D4C5B0', color: '#3E3832', border: '1px solid #C5B89A' }
        },
      ...
      h('header', { className: 'mb-4' },
        ...
        h('p', { className: 'text-center subtitle', style: { color: '#6B6560' } }, 'Manage your crafting macros and recipes')
      ),
      h('div', { className: 'mb-6 space-y-3' },
        ...
        h('div', { className: 'flex items-center' },
          h('button', {
            ...
            className: 'px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1.5 text-sm ml-auto',
            style: { backgroundColor: '#8B6F47', color: 'white' }
          },
      ...
      h('div', { className: 'space-y-2 pb-8' },
        sortedCrafts.length === 0
          ? h('div', { className: 'text-center py-12', style: { color: '#6B6560' } },
      ...
      h('form', { onSubmit: handleSubmit, className: 'space-y-4' },
        ...
        h('div', { className: 'flex gap-2 justify-end' },
          h('button', {
            ...
            style: { backgroundColor: '#D4C5B0', color: '#3E3832' }
          }, 'Cancel'),
          h('button', {
            type: 'submit',
            className: 'px-4 py-2 rounded-lg transition-colors',
  ...
  const CraftCard = ({ craft, classStats, isExpanded, onToggle, onEdit, onDelete }) => {
    ...
    const header = h('div', {
      className: `flex items-center p-4 ${isExpanded ? 'border-b' : ''} hover:opacity-95 transition-colors cursor-pointer`,
      onClick: onToggle,
      ...
      h('div', { className: 'flex items-center gap-4 ml-auto' },
        h('div', { className: 'flex items-center gap-2' },
          h('span', { className: 'material-icons text-sm', style: { color: '#6B6560' } }, CLASS_ICONS[craft.class] || 'build'),
          h('span', { className: 'text-sm uppercase', style: { color: '#6B6560' } }, craft.class)
        ),
        craft.level && h('span', { className: 'text-sm', style: { color: '#6B6560' } }, `LVL ${craft.level}`),
        h('span', {
        ...
        h('button', {
          ...
          onMouseOver: (e) => e.target.style.backgroundColor = '#5d1515',
          onMouseOut: (e) => e.target.style.backgroundColor = '#7d1d1d',
          title: 'Delete'
        },
    ...
    return h('div', { className: 'craft-card rounded-lg', style: { overflow: 'visible' } },
      ...
      isEditingInline
        ? h('div', {
            ...
            h('div', { className: 'flex items-center gap-2' },
              h('span', { className: 'material-icons text-sm', style: { color: '#3E3832' } }, CLASS_ICONS[editFormData.class] || 'build'),
              h('span', { className: 'font-medium uppercase', style: { color: '#3E3832' } }, editFormData.class),
              h('span', { className: 'text-xs ml-2', style: { color: '#6B6560' } }, '(Edit class stats for this craft)')
            ),
            h('div', { className: 'grid grid-cols-3 gap-3' },
              h('div', null,
                h('label', { className: 'block text-xs mb-1', style: { color: '#3E3832' } }, 'Craftsmanship'),
                h('input', {
                  type: 'number',
                  value: editFormData.classStats?.craftsmanship || 0,
                  onChange: (e) => updateClassStat('craftsmanship', e.target.value),
                  className: 'w-full px-2 py-1 rounded text-sm',
                  style: { backgroundColor: 'white', border: '1px solid #C5B89A', color: '#3E3832' }
                })
              ),
              h('div', null,
                h('label', { className: 'block text-xs mb-1', style: { color: '#3E3832' } }, 'Control'),
                h('input', {
                  type: 'number',
              ...
              h('div', null,
                h('label', { className: 'block text-xs mb-1', style: { color: '#3E3832' } }, 'CP'),
                h('input', {
                  type: 'number',
        ...
        : h('div', {
            ...
            h('div', { className: 'flex items-center gap-2' },
              h('span', { className: 'material-icons text-sm', style: { color: '#3E3832' } }, CLASS_ICONS[craft.class] || 'build'),
              h('span', { className: 'font-medium uppercase', style: { color: '#3E3832' } }, craft.class)
            ),
            h('div', { className: 'flex items-center gap-4 text-xs', style: { color: '#6B6560' } },
              h('span', null,
                h('span', null, 'Craftsmanship: '),
                h('span', { className: 'font-medium', style: { color: '#3E3832' } }, stats.craftsmanship)
              ),
              h('span', null,
                h('span', null, 'Control: '),
                h('span', { className: 'font-medium', style: { color: '#3E3832' } }, stats.control)
              ),
              h('span', null,
                h('span', null, 'CP: '),
                h('span', { className: 'font-medium', style: { color: '#3E3832' } }, stats.cp)
              )
            )
          ),
      // Body
      h('div', { className: 'p-4 space-y-4' },
        isEditingInline
          ? // Inline editing form
            h('div', { className: 'space-y-4' },
              // Craft Name
              h('div', null,
                h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Craft Name *'),
                h('input', {
                  type: 'text',
              ...
              h('div', { className: 'grid grid-cols-2 md:grid-cols-3 gap-4' },
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Class'),
                  h('select', {
                ...
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Level'),
                  h('input', {
                    type: 'number',
                ...
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Rating'),
                  h('select', {
                ...
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Durability'),
                  h('input', {
                    type: 'number',
                ...
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Difficulty'),
                  h('input', {
                    type: 'number',
                ...
                h('div', null,
                  h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Quality'),
                  h('input', {
                    type: 'number',
              ...
              h('div', { className: 'grid grid-cols-1 gap-4' },
                h('div', { className: 'space-y-2' },
                  h('div', { className: 'flex items-center gap-2' },
                    ...
                    h('label', { htmlFor: 'editRequiresFood', className: 'text-sm font-medium', style: { color: '#3E3832' } }, 'Requires Food')
                  ),
                ...
                h('div', { className: 'space-y-2' },
                  h('div', { className: 'flex items-center gap-2' },
                    ...
                    h('label', { htmlFor: 'editRequiresPotion', className: 'text-sm font-medium', style: { color: '#3E3832' } }, 'Requires Potion')
                  ),
              ...
              // Macro
              h('div', null,
                h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Macro'),
                h('textarea', {
              ...
              // Notes
              h('div', null,
                h('label', { className: 'block text-sm font-medium mb-2', style: { color: '#3E3832' } }, 'Notes'),
                h('textarea', {
        ...
        h('div', { className: 'flex flex-wrap gap-6' },
          ...
          (craft.durability || craft.difficulty || craft.quality) && h('div', { className: 'flex gap-4 text-sm' },
            craft.durability && h('span', null,
              h('span', { style: { color: '#6B6560' } }, 'Durability: '),
              h('span', { className: 'font-medium', style: { color: '#3E3832' } }, craft.durability)
            ),
            craft.difficulty && h('span', null,
              h('span', { style: { color: '#6B6560' } }, 'Difficulty: '),
              h('span', { className: 'font-medium', style: { color: '#3E3832' } }, craft.difficulty)
            ),
            craft.quality && h('span', null,
              h('span', { style: { color: '#6B6560' } }, 'Quality: '),
              h('span', { className: 'font-medium', style: { color: '#3E3832' } }, craft.quality)
            )
          ),
          ...
          (craft.requiresFood || craft.requiresPotion) && h('div', { className: 'flex gap-4 text-sm' },
            craft.requiresFood && h('div', { className: 'flex items-center gap-2' },
              h('span', { className: 'material-icons text-sm', style: { color: '#D4A574' } }, 'restaurant'),
              h('span', { style: { color: '#3E3832' } }, craft.food)
            ),
            craft.requiresPotion && h('div', { className: 'flex items-center gap-2' },
              h('span', { className: 'material-icons text-sm', style: { color: '#8B6F47' } }, 'science'),
              h('span', { style: { color: '#3E3832' } }, craft.potion)
            )
          )
        ),
        // Macros
        craft.macro && h('div', null,
          h('h4', { className: 'font-semibold mb-2 text-sm', style: { color: '#3E3832' } }, 'Macros:'),
            ...
            ...macroChunks.map((chunk, index) =>
              h('div', { key: index, className: 'relative' },
                h('div', { className: 'flex justify-between items-center mb-1' },
                  h('span', { className: 'text-xs', style: { color: '#6B6560' } }, `Macro ${index + 1}`),
                  h('button', {
                ...
                h('pre', {
                  ...
                  style: { backgroundColor: 'white', color: '#3E3832', border: '1px solid #C5B89A' }
                }, chunk)
              )
            )
          )
        ...
        craft.notes && h('div', null,
          h('h4', { className: 'font-semibold mb-2 text-sm', style: { color: '#3E3832' } }, 'Notes:'),
          h('p', { className: 'text-sm', style: { color: '#3E3832' } }, craft.notes)
        )
            )
      )
    );
  ...
  const ClassStatsManager = ({ classStats, onUpdate, onClose }) => {
    ...
    return h('div', {
      ...
      h('div', {
        ...
        h('p', { className: 'text-sm mb-4', style: { color: '#6B6560' } }, 'These stats will be displayed for all crafts using each class.'),
        h('div', { className: 'space-y-4 mb-6' },
          ...CLASSES.map(className =>
            h('div', {
              key: className,
              className: 'p-4 rounded-lg',
              style: { backgroundColor: '#e3ded3', border: '1px solid #C5B89A' }
            },
              h('h3', { className: 'font-semibold mb-3', style: { color: '#3E3832' } }, className),
